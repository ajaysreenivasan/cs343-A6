...
void uMain::main(){
	...
	processConfigFile(inputFile,cp);
																							//create specified tasks in correct order
    ...
    for(unsigned int i =0; i< cp.numVendingMachines; i++){
    	VendingMachine * tempVendingMachine = new VendingMachine(*printer,*nameServer,i,
    										cp.sodaCost,cp.maxStockPerFlavour);

        vendingMachines.push_back(tempVendingMachine);
    }
